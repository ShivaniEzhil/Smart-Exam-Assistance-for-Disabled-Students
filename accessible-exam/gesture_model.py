...
class GestureModel:
    ...
    def process_frame(self, frame):
        letter, _ = self.process_frame_with_confidence(frame)
        return letter
    ...
    def process_frames_vote(self, frames, min_votes=2):
        ...
        if count < min_votes and len(votes) > 1:
            return None
        return best_letter

    def process_frame_with_confidence(self, frame):
        """Return (letter, confidence) or (None, 0). Returns (None, conf) if hand is detected but confidence is low."""
        ...
        try:
            ...
            keypoints_raw = np.array([[lm.x, lm.y, lm.z] for lm in landmarks]).astype(np.float64)
            if keypoints_raw.size != 63:
                return None, 0.0

            # 1. Check original orientation
            kp_norm = normalize_landmarks(keypoints_raw.flatten())
            pred = self.ml_model.predict([kp_norm])[0]
            conf = float(np.max(self.ml_model.predict_proba([kp_norm]))) if hasattr(self.ml_model, 'predict_proba') else 0.0

            # 2. Check horizontally flipped orientation (supports left hand & mirrored webcam)
            kp_flipped = keypoints_raw.copy()
            kp_flipped[:, 0] = -kp_flipped[:, 0]
            kp_flipped_norm = normalize_landmarks(kp_flipped.flatten())
            if hasattr(self.ml_model, 'predict_proba'):
                probs_flipped = self.ml_model.predict_proba([kp_flipped_norm])[0]
                conf_flipped = float(np.max(probs_flipped))
                if conf_flipped > conf:
                    pred = self.ml_model.predict([kp_flipped_norm])[0]
                    conf = conf_flipped
            ...
            # If confidence is below threshold, return None for letter while still reporting confidence > 0
            if conf < 0.20:
                return None, conf
